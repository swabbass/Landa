...
public class Reciever extends BroadcastReceiver implements PostListener {
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (intent.getAction().toString()
			...
			Update u = Utilities.generateUpdateFromExtras(intent.getExtras(),
					context);
			if (u != null&& u.getUrlToJason()==null) {
			dbmngr.insertUpdate(u);
			Utilities.showNotification(context, u.getSubject(), u.getText());
			}
			else {
				Utilities.fetchUpdateFromBackEndTask task=new Utilities.fetchUpdateFromBackEndTask(context, this);
		...
		if(intent.getAction().equals(Settings.WARD_LANDA_ALARM))
		{
			...
			String s=String.format("מקום : %s \n שעה : %s", c.getPlace(),c.getTimeFrom());
